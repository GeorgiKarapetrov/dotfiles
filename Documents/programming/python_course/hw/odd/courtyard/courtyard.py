...
class Courtyard:
    ...
    def find_freest_place(self, resol: int=100):
        """Return the coordinates of the point with the most free space around. Default grid is spaced resolution is 10 000 attempts"""

        from itertools import product as prod

        largest_distance = 0
        for i,j in prod(range(resol), range(resol)):
            test_v = Vector( self.width * (i / resol), self.height * (j / resol))
            test_distance = self.calc_min_distance(test_v.x, test_v.y)
            if test_distance > largest_distance:
                largest_distance = test_distance
                freest_place = test_v
        return freest_place
